...
#include "Chat/ChatItemDelegate.h"
...
ChatWidget::ChatWidget(const QString& id, QWidget* parentWidget)
    ...
    , _chatFile(_file)
{
    Q_ASSERT(!_chatDirectory.isEmpty()
             && "Please use setChatDirectory() before creating any ChatWidget");
...
void
ChatWidget::addMessage(const QString& nickName,
...
void
ChatWidget::addItem(ChatItem* item, bool saveToHistory)
{
...
void
ChatWidget::setChatDirectory(const QString& chatDirectory)
{
...
ChatWidget::resizeEvent(QResizeEvent*)
{
    ui->tableView->setColumnWidth(
        1, ui->tableView->width() - (NICKNAME_COLUMN_WIDTH + TIME_COLUMN_WIDTH + 60));
    ui->tableView->resizeRowsToContents();
}

void
ChatWidget::showEvent(QShowEvent* event)
{
    if (!_showed) {
        _showed = true;
        resizeEvent(0);
