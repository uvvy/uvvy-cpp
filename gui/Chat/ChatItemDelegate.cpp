...
void
ChatItemDelegate::paint(QPainter* painter,
...
QSize
ChatItemDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    if (_model != 0) {
        if (index.isValid()) {
            std::shared_ptr<ChatItem> item = _model->item(index.row());
...
QWidget*
ChatItemDelegate::createEditor(QWidget* parent,
                               const QStyleOptionViewItem& option,
                               const QModelIndex& index) const
{
    if (_model != 0) {
