...
{
protected:
	host* const host; ///< Per-host state.
	stream* owner; ///< Back-pointer to stream object, or nullptr if stream has been deleted.
...
public:
	...
	virtual int write_record(const char* data, size_t size) {
		return write_data(data, size, dataMessageFlag);
	}
	 ...
	 */
	inline void listen(listen_mode mode) {
		listen_mode_ = mode;
	}
	inline listen_mode listen_mode() {
		return listen_mode_;
	}
	/**
