...
{
    qDebug() << this << "acked packet of size" << pkt.buf.size();
    ...
    case DataPacket:
        ...
        if (twait.remove(pkt.tsn)) {
            ...
            qDebug() << "twait remove" << pkt.tsn << "size" << pkt.payloadSize()
                     << "new cnt" << twait.size() << "twaitsize" << twaitsize;
        }
...
bool BaseStream::missed(StreamFlow *, const Packet &pkt)
{
    ...
    qDebug() << this << "missed bsn" << pkt.tsn << "size" << pkt.buf.size();

    switch (pkt.type) {
    case DataPacket:
        qDebug() << this << "retransmit bsn" << pkt.tsn << "size" << pkt.buf.size() - hdrlenData;
    ...
    case AttachPacket:
        qDebug() << this << "Attach packet lost: trying again to attach";
        txenqflow();
        return true;

    case DatagramPacket:
        qDebug() << this << "Datagram packet lost: oops, gone for good";
...
void BaseStream::endflight(const Packet &pkt)
{
    ...
    if ((hdr->type >> typeShift) == InitPacket) {
        if (parent) {
            ...
            qDebug() << this << "endflight" << pkt.tsn << "bytes in flight (parent)" << parent->tflt;
            Q_ASSERT(parent->tflt >= 0);
        }
    } else {    // Reply or Data packet
        tflt -= pkt.payloadSize();
        qDebug() << this << "endflight" << pkt.tsn << "bytes in flight" << tflt;
    }
    // XXX Q_ASSERT(tflt >= 0);
}
        ...
        quint16 sid, unsigned slot, const UniqueStreamId &usid)
{
    ...
    qDebug() << this << "accepting stream" << usid << "as" << nbs;
...
// Helper function to enqueue useful rseg data.
void BaseStream::rxEnqueueRseg(const RxSegment& rseg, int actsize, /*inout*/ bool& closed)
{
    rsegs.enqueue(rseg);
    rsn += actsize;
    ravail += actsize;
    rmsgavail += actsize;
    rbufused += actsize;
    if ((rseg.flags() & (dataMessageFlag | dataCloseFlag)) and (rmsgavail > 0))
    {
...
{
    qDebug() << this << "rxData" << byteseq << "size" << (pkt.size() - hdrlenData);
    ...
    if (endread) {
        ...
        qDebug() << this << "Ignoring segment received after end-of-stream";
    ...
    // For now, whine but accept it anyway... (XXX)
    ...
    if (rsndiff <= 0) {
        ...
        if (actsize < 0 || (actsize == 0 && !rseg.hasFlags())) {
            ...
            qDebug() << this << "Duplicate segment at RSN" << rseg.rsn << "size" << segsize;
            goto done;
        }
        rseg.hdrlen -= rsndiff; // Merge useless data into "headers"
        qDebug() << this << "actsize" << actsize << "flags" << rseg.flags();
        ...
        rxEnqueueRseg(rseg, actsize, closed);
...
void BaseStream::dump()
{
    ...
    qDebug() << "Stream" << this << "state" << state
             << "TSN" << tasn << "tqueue" << tqueue.size()
             << "RSN" << rsn << "ravail" << ravail
