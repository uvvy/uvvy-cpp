...
private:
    enum State {
        ...
        WaitService,    // Initiating, waiting for service reply
    ...
    struct RxSegment {
        ...
        inline const StreamHeader *constHdr() const {
            return (const StreamHeader*) (buf.constData() + Flow::hdrlen);
        }
...
private:
    ...
    void rxEnqueueRseg(const RxSegment& rseg, int actsize, /*inout*/ bool& closed);
...
public:
    ...
    virtual qint64 bytesToWrite() const {
        return twaitsize; // XXX dgrams
    }

    virtual int readData(char *data, int maxSize);
    virtual int writeData(const char *data, int maxSize, quint8 endflags);
