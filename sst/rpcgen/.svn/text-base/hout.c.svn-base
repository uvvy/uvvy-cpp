...
/*
 * hout.c, Header file outputter for the RPC protocol compiler
...
void
print_xdr_func_def(name)
char* name;
{
	// XXX make namespace selectable/optional
	f_print(fout, "SST::XdrStream &operator<<(SST::XdrStream &xs, const SST::%s &v);\n",
		name);
	f_print(fout, "SST::XdrStream &operator>>(SST::XdrStream &xs, SST::%s &v);\n\n",
		name);
}
	...
	definition *def;
{
	...
	for (l = def->def.un.cases; l != NULL; l = l->next) {
	    if (l->contflag == 0)
		pdeclaration(name, &l->case_decl, 1, ";\n");
	}
	decl = def->def.un.default_decl;
	if (decl && !streq(decl->type, "void")) {
		pdeclaration(name, decl, 1, ";\n");
	}
	f_print(fout, "};\n");
}

static void
pcastdefine(name, type, num)
	char *name;
	char *type;
	char *num;
{
	f_print(fout, "#define\t%s ((%s)(%s))\n", name, type, num);
}
...
pfreeprocdef(char * name, char *vers, int mode)
{
	...
	pvname(name, vers);
	if (mode == 1)
		f_print(fout,"_freeresult(SVCXPRT *, xdrproc, caddr_t);\n");
	else
	...
	definition *def;
{
	...
	pcastdefine(def->def_name, "rpcprog_t", def->def.pr.prog_num);
	for (vers = def->def.pr.versions; vers != NULL; vers = vers->next) {
		if (tblflag) {
			f_print(fout,
		...
		pcastdefine(vers->vers_name, "rpcvers_t", vers->vers_num);
...
/*XXX			pfreeprocdef(def->def_name, vers->vers_num, i);*/
			}
	...
	definition *def;
{
	...
	if (!streq(name, old)) {
		...
		case REL_VECTOR:
		case REL_ARRAY:
			f_print(fout, "QList<%s%s> %s", prefix, old, name);
			break;
		case REL_POINTER:
			f_print(fout, "XdrPointer<%s%s> %s", prefix, old, name);
			break;
		case REL_OPTION:
			f_print(fout, "XdrOption<%s%s> %s", prefix, old, name);
			break;
		case REL_ALIAS:
	...
	char *separator;
{
	...
	if (streq(dec->type, "string")) {
		f_print(fout, "QString %s", dec->name);
	} else {
		prefix = "";
		...
		case REL_ARRAY:
			if (streq(dec->type, "opaque"))
				f_print(fout, "QByteArray %s", dec->name);
			else
				f_print(fout, "QList<%s%s> %s",
					prefix, type, dec->name);
			break;
		case REL_POINTER:
			f_print(fout, "XdrPointer<%s%s> %s",
				prefix, type, dec->name);
			break;
		case REL_OPTION:
			f_print(fout, "XdrOption<%s%s> %s",
				prefix, type, dec->name);
			break;
		}
	}
