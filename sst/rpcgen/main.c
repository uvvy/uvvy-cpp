...
/*
 * main.c, Top level of the RPC protocol compiler.
...
int inlinemin = INLINEMAX;
/*
 * Length at which to start doing an inline. INLINEMAX = default
	...
	char *argv[];
{
	...
	if (cmd.cflag) {
		...
		c_output(cmd.infile, "-DRPC_XDR", EXTEND, "_xdr.cc");
		reinitialize();
		h_output(cmd.infile, "-DRPC_HDR", EXTEND, ".h");
		reinitialize();
#if 0
		l_output(cmd.infile, "-DRPC_CLNT", EXTEND, "_clnt.cc");
		reinitialize();
	...
	char *outfile;
{
	...
	if (infile && (include = extendfile(infile, ".h"))) {
		...
		free(include);
		/* .h file already contains xdr.h */
	} else
		f_print(fout, "#include \"xdr.h\"\n");

	// XXX make namespace optional, selectable
	f_print(fout, "\nusing namespace SST;\n");
...
struct commandline *cmd;
{
	...
	svcname = file_name(cmd->infile, "_svc.cc");
	clntname = file_name(cmd->infile, "_clnt.cc");
	xdrname = file_name(cmd->infile, "_xdr.cc");
		...
		if (argv[i][0] != '-') {
			...
			for (j = 1; argv[i][j] != 0; j++) {
				...
				case 'Y':
					...
					if (snprintf(pathbuf, sizeof(pathbuf), "%s/cpp", argv[i]) >= (int)sizeof(pathbuf)) {
